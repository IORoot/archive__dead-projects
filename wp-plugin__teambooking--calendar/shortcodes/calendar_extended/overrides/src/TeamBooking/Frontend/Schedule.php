        ...
        foreach ($this->slots as $slot) {
            ...
            if ($this->params->getAltSlotStyle() === 1) {
                ...
                $attributes_to_add .= ' style="border: 2px solid '
                    . Toolkit\hex2RGBa($service->getColor(), 0.5)
                    . ';background: ' . Toolkit\hex2RGBa($service->getColor(), 1) . '"';
            }
            if (!$slot->isSoldout()) {
            ...
            // ANDYP - Image & Golden Grid
            ?>
            <div class="goldengrid"><div class="image image__<?= strtolower(str_replace(' ', '-', esc_html($slot->getServiceName(TRUE)))) ?>"></div></div>
            </div>
            <?php
                
                /**
                 * ANDYP - Custom action to build schema on slot
                 */
                $args = [
                    'type'      => 'Event',
                    'id'        => $slot->getServiceId(),
                    'title'     => $slot->getServiceName(),
                    'info'      => trim(preg_replace('/\s+/', ' ', strip_tags($slot->getServiceInfo()))),
                    'start'     => $slot->getDateFormatted(DATE_ATOM, 'start'),
                    'startdate' => $slot->getDateFormatted('d-m-Y', 'start'),
                    'end'       => $slot->getDateFormatted(DATE_ATOM, 'end'),
                    'location'  => $slot->getLocation(),
                    'price'     => $service->getPrice(),
                ];
                do_action( 'build_schema', $args );
